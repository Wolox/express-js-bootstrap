const { flatten } = require('lodash'),
  ...
  { exampleProjects, basicFiles, basicFilesGraphql, exampleProjectsGraphql } = require('./helpers/constants');
...
const testSnapshot = technology => (projectName, { kickoffOptions, files, templateFiles }) => {
  ...
  test.each(flatten([...templateFiles, technology === 'graphQL' ? basicFilesGraphql : basicFiles]))(
    'creates expected %s',
    file => {
...
describe.each(exampleProjects)('Example project with %s', testSnapshot('expressJS'));
describe.each(exampleProjectsGraphql)('Example project with %s', testSnapshot('graphQL'));
